...
def _get_streak_multipliers_per_deck(mw: aqt.main.AnkiQt, config: dict, card: anki.cards.Card) -> dict:
...
def _get_streak_multipliers_per_card(mw: aqt.main.AnkiQt, config: dict, card: anki.cards.Card) -> dict:
    current_epoch_time = int(datetime.now(tz=timezone.utc).timestamp() * 1000)
    allowed_epoch_time = str(current_epoch_time - config["streak"]["streak_time_epoch_ms"])
    # cid = card_id, id = epoch_ms_timestamp, ease = answer_button_pressed https://github.com/ankidroid/Anki-Android/wiki/Database-Structure#review-log
    ...
    for config_device in config["devices"]:
        ...
        if len(enabled_actuators) > 0:
            ...
            if config["streak"]["streak_type"] == "Per Card":
                streak_multipliers = _get_streak_multipliers_per_card(mw, config, card)
            elif config["streak"]["streak_type"] == "Per Deck":
                streak_multipliers = _get_streak_multipliers_per_deck(mw, config, card)
